...
```{r v2gEstimates, fig.caption = "Estimates of available kWh by time period"}
...
These show, for example, that the mean kWh available during the evening peak period is ~`r dt[chargeType %like% "Standard" & peakPeriod %like% "Evening", mean(kWh)]` with a range of `r dt[chargeType %like% "Standard" & peakPeriod %like% "Evening", round(min(kWh,2))]` to 'r dt[chargeType %like% "Standard" & peakPeriod %like% "Evening", round(max(kWh),2))]` kWh. We can also see that similar values are available in the morning peak period but further analysis would be required to estimate how much of this could be used during the period without impacting subsequent use. Other analyses could include:
