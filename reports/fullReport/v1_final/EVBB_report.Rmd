---
params:
  authors: 'Rafferty Parker and Ben Anderson ([Centre for Sustainability](https://www.otago.ac.nz/centre-sustainability/), University of Otago, ben.anderson@otago.ac.nz); Daniel Myall (Flip The Fleet, daniel@zeno.nz)'
...
rawStartDate <- min(lubridate::date(rawDT$r_dateTime),na.rm = TRUE)
rawEndDate <- max(lubridate::date(rawDT$r_dateTime), na.rm = TRUE)
...
cleanDT[, chargeType := ifelse(charge_power_kw > kwThreshold, "Rapid charging", chargeType)]
...
cleanDT <- setPeakPeriod(cleanDT)

# set for sequence extraction
cleanDT[, startTime := r_dateTime]
```
...
cleanSampleStart <- "2018-10-01"
cleanSampleEnd <- "2019-01-16"

# no more processing of cleanDT after this ----
cleanDT <- cleanAllDT[as.Date(r_dateTime) >= as.Date(cleanSampleStart) & 
                        as.Date(r_dateTime) < as.Date(cleanSampleEnd)]

cleanStartDate <- min(lubridate::date(cleanDT$r_dateTime),na.rm = TRUE)
cleanEndDate <- max(lubridate::date(cleanDT$r_dateTime), na.rm = TRUE)

rawCap <- paste0("Data source: Flip The Fleet (original data)",
                 "\n Period: ", cleanStartDate, " - ", cleanEndDate)
...
> __The analysis reported below is work in progress. Please check with the authors before quoting or re-using (with attribution) results/plots. Reporting interim results without checking can seriously damage your health (and career :-)__. [#YMMV](https://www.urbandictionary.com/define.php?term=ymmv).
...
The original supplied data consisted of `r tidyNum(nrow(rawDT))` observations for `r uniqueN(rawDT$id)` EVs for the period `r rawStartDate` to `r rawEndDate`.
...
Figure \@ref(fig:rawObsChargingPattern) plots the number of observations per half hour by EV for the raw data. It appears to suggest that there is a large gap in the charging observations in February. This is not easily explainable as non-charging (i.e. driving) observations continue through this period.

```{r rawObsChargingPattern, fig.cap="Observed charging (raw data)"}
rawDT[, r_dateTimeHalfHour := lubridate::floor_date(r_dateTime, unit = "30 minutes")]
rawDT[, charging := ifelse(charge_power_kw > 0, "Charging > 0 kW (charging)", "Charging == 0 kW (not charging)")]
plotDT <- rawDT[, 
                  .(meankW = mean(charge_power_kw),
                    nObs = .N),
                  keyby = .(dvID, r_dateTimeHalfHour, charging)]

myCap <- paste0(rawCap,
                 "\n All observations",
                "\n n observations used: ", tidyNum(nrow(rawDT)),
                "\n n EVs: ", uniqueN(plotDT$dvID)
)

p <- ggplot2::ggplot(plotDT, aes(x = r_dateTimeHalfHour, y = dvID, fill = nObs)) +
  geom_tile() +
  facet_grid(. ~ charging , scales = "free")
p + labs(y = "Vehicle",
       x = "Date",
       caption = myCap) +
...
Figure \@ref(fig:initialDataChecksPlotDate) shows the unique number of EVs recorded on each day by whether or not they were charging and reflects the period over which Flip The Fleet installed the data collection boxes. It also shows the unexplained drop in charging observations during February.

```{r initialDataChecksPlotDate, fig.cap="Number of unique EVs observed by time of day and date"}
dt <- rawDT[, .(nEVs = uniqueN(id),
                    nObs = .N), keyby = .(charging, date = lubridate::date(r_dateTime))]

myCap <- paste0(rawCap,
                "\n n observations used: ", tidyNum(nrow(rawDT)),
                "\n Shaded area = data selection for subsequent analysis")

p <- ggplot2::ggplot(dt, aes(x = date, y =  nEVs, colour = charging)) +
  geom_line() +
  #scale_alpha(breaks = c(0,5,10,15,20)) +
  labs(x = "Date",
...
p + annotate("rect", xmin = as.Date(cleanSampleStart), # cut
             xmax = as.Date(cleanSampleEnd), # set earlier
...
vehicleIDsDT <- rawDT[, .(nObs = .N,
                          ...
                          meankWCharging = mean(charge_power_kw),
                          maxkWCharging = max(charge_power_kw)), keyby = .(id)]
...
kableExtra::kable(t[!is.na(Year)], digits = 2, caption = "Descriptive statistics for charging (kW, clean data) ") %>%
  kable_styling()
```
...
Charging data has been broadly separated into two separate categories, 'Standard' and 'Rapid'. Standard charging is defined to be when the charger is reading less than or equal to `r kwThreshold` kW - this is considered the upper limit of ordinary home charging without an expensive wiring upgrade [@ConceptConsulting2018]. Rapid charging is defined as all charging greater than `r kwThreshold` kW, and would probably occur at designated and purpose-built public charging stations.

It should be noted that this method is not always accurate since we can identify apparent sequences of charging which start at > `r kwThreshold` kW and decline to < `r kwThreshold` kW over a relatively short period or vice versa (see Section \@ref(chargeFlagTest)). In this circumstance the first observation will be correctly classified as 'Rapid' but the lower observations, which we assume are lower power 'top-ups' at the end of a rapid charge will be incorrectly classified as 'Standard'.  As an example, we know that there are `r nrow(firstLastDT[chargeTypeError %like% "Error"])` sequences of charging events (out of a total of `r nrow(firstLastDT)`) where the first and last charge types do not match. 
...
p <- ggplot2::ggplot(plotDT, aes(x = charge_power_kw, fill = chargeType)) +
  ...
  geom_vline(xintercept = kwThreshold)

p + labs(y = "Charging observations",
       x = "Power (kW)",
       caption = paste0(myCap, "\nCharging threshold = ", kwThreshold, " kW")) +
  guides(fill = guide_legend(title = "Inferred charge type:")) +
  scale_fill_manual(values=cbgPalette) + # use colour-blind friendly palette
  theme(legend.position = "bottom") 
```

Finally, we test the temporal distribution of observed charging by aggregating to half-hours. Figure \@ref(fig:cleanObsChargingPattern) gives some indication of the relative prevalence of each charging type.

```{r cleanObsChargingPattern, fig.cap="Observed charging"}
cleanDT[, r_dateTimeHalfHour := lubridate::floor_date(r_dateTime, unit = "30 minutes")]

plotDT <- cleanDT[!(chargeType %like% "Not charging"), 
                  .(meankW = mean(charge_power_kw),
                    nObs = .N),
                  keyby = .(dvID, r_dateTimeHalfHour, chargeType)]
...
p <- ggplot2::ggplot(plotDT, aes(x = r_dateTimeHalfHour, y = dvID, fill = meankW)) +
  geom_tile() +
  facet_grid(. ~ chargeType , scales = "free")
p + labs(y = "Charging observations",
       x = "Power (kW)",
       caption = myCap) +
  scale_fill_continuous(low = "green", high = "red") + # use colour-blind friendly palette
  theme(legend.position = "bottom")
```
...
p <- ggplot(plotDT[!(chargeType %like% "Not")], aes(x = qHour, y = nDaily, colour = chargeType)) +
  geom_line() +
  facet_grid(weekdays ~ .) +
  theme(legend.position="bottom") +
  guides(colour = guide_legend(title = "Charge type"))
p <- p + labs(x = "Time of day", y = "Count per 15 minutes")

yMin <- min(plotDT$nDaily, na.rm = TRUE)
yMax <- max(plotDT$nDaily, na.rm = TRUE)
p <- addPeaks(p)


ggplot2::ggsave(paste0(pPath,"/chargeTimeCount.png"), p ) # for PCE report
...
```{r chargeTimeCountLog, fig.cap="log(Frequency) plot of charging start times during weekdays"}
p + scale_y_log10() + annotation_logticks() # so we can see rapid charge patterns
```
...
The lack of data from EVs which are neither charging nor being driven means that it is impossible to _directly_ calculate the proportion of the sample who were charging at a given time. Instead we can _estimate_ the proportion of EVs which were charging by:
...
```{r pcCharging, fig.cap="Percent of vehicles charging in a given 15 minute period"}
...
evPcChargingPlotDT <- t[, .(meanPC = mean(pcEVs)), keyby = .(weekdays, qHour, chargeType, peakPeriod)] # we need this one later for various calculations

p <- ggplot2::ggplot(evPcChargingPlotDT, aes(x = qHour, y = 100 * meanPC, colour = chargeType)) +
  geom_point() +
  scale_color_discrete(name = "Charge Type") +
  labs(y = "Mean % of EVs per quarter hour", x = "Time of day") +
  facet_grid(weekdays ~ .)

yMin <- min(evPcChargingPlotDT$meanPC * 100)
yMax <- max(evPcChargingPlotDT$meanPC * 100)
p <- addPeaks(p)

ggplot2::ggsave(paste0(pPath,"/pcCharging.png"), p ) # for PCE report

p
```

These results are reported in Figure \@ref(fig:pcCharging) which shows the estimated mean % of EVs which were charging in each quarter-hour of the day by charge type. As we would expect the pattern of charging replicates that found in Figure \@ref(fig:chargeTimeCount) and indicates the relative rarity of rapid charging.
...
reportSocTable <- function(dt){
  t <- dt[, .("Mean SoC %" = mean(SoC_percent),
              "Median SoC %" = median(SoC_percent),
              "n Obs" = .N,
              "n EVs" = uniqueN(id)),
          keyby = .("Charge Type" = chargeType, "Period" = peakPeriod)
          ]
  return(t)
}

makeSocPlot <- function(dt,version){
  myCap <- paste0(cleanCap, 
                  "\n First charging value in a sequence",
                  "\n n observations: ", tidyNum(nrow(dt)),
                  "\n n EVs: ", uniqueN(dt$dvID)
  )
  p <- ggplot(dt, aes(x = SoC_percent)) + 
    geom_histogram(bins = 20) +
    facet_grid(chargeType ~ peakPeriod, scales = "free")
  
  p <- p + labs(x = "State of charge when charging begins (%)")
  
  ggplot2::ggsave(paste0(pPath,"/SocHistogram", version,".png"), p ) # for PCE report
  
  p <- p + 
    labs(x = "State of charge when charging begins (%)", 
                caption = myCap) # have to re-set the labels option
  return(p)
}

dt <- cleanDT[chargeFlag %like% "First"]

kableExtra::kable(reportSocTable(dt), digits = 2, 
                  caption = "Summary statistics for % state of charge on first charge in a sequence") %>%
  kable_styling()

makeSocPlot(dt, version = "firstAll")
...
dt <- cleanDT[chargeFlag %like% "First" & SoC_percent < 90]

kableExtra::kable(reportSocTable(dt), digits = 2, 
                  caption = "Summary statistics for % state of charge on first charge in a sequence, values > 90% removed") %>%
  kable_styling()

makeSocPlot(dt, version = "first90pc")
```

Figure \@ref(fig:SoCplot2corrected) repeats this analysis but uses the cleaned and corrected inferred start/end of charging sequence data instead of all charging observations. Figure \@ref(fig:SoCplot3) shows very similar distributions to the previous 'all-observations' plot (Figure \@ref(fig:SoCplot1)) and confirms that sequences of standard charging in particular most frequently start with battery state of charge over 50%. 

```{r SoCplot2corrected, fig.cap="Value of state of charge at beginning of charging sequence (chargeType corrected, values > 90% removed)"}
dt <- firstCleanDT[SoC_percent < 90]

kableExtra::kable(reportSocTable(dt), digits = 2, 
                  caption = "Summary statistics for % state of charge on first charge in a sequence, chargeType corrected, values > 90% removed") %>%
  kable_styling()

makeSocPlot(dt, version = "firstCorrected90pc")

# % of rapid starting at 5% or less
dt[, cut := ifelse(SoC_percent <= 5, "<= 5%", "Other")]
dt[, cut := ifelse(SoC_percent > 50, "> 50%",cut)]
t <- dt[, .(nObs = .N), keyby = .(cut, chargeType)]

# test CI
# p +/- Z * sqrt((p(1-p)/n))
#z = 1.96 (95%)

p <- t[cut == "> 50%" & chargeType %like% "Rapid", nObs]/t[chargeType %like% "Rapid", sum(nObs)]

n <- uniqueN(dt$dvID)
ci_u <- p + 1.96*sqrt((p*(1-p)/n))
ci_l <- p - 1.96*sqrt((p*(1-p)/n))

```

Statistics:

 * `r t[cut == "<= 5%" & chargeType %like% "Rapid", nObs]` of the `r t[chargeType %like% "Rapid", sum(nObs)]` rapid charging sequences with intial SoC < 90% started with less than 5% charge;
 * `r t[cut == "> 50%" & chargeType %like% "Rapid", nObs]` of `r t[chargeType %like% "Rapid", sum(nObs)]` (`r round(100*(t[cut == "> 50%" & chargeType %like% "Rapid", nObs]/t[chargeType %like% "Rapid", sum(nObs)]),2)`%) rapid charging sequences with intial SoC < 90% started with greater than 50% charge;
  * `r t[cut == "> 50%" & chargeType %like% "Standard", nObs]` of `r t[chargeType %like% "Standard", sum(nObs)]` (`r round(100*(t[cut == "> 50%" & chargeType %like% "Standard", nObs]/t[chargeType %like% "Standard", sum(nObs)]),2)`%) standard charging sequences with intial SoC < 90% started with greater than 50% charge;



```{r SoCplotTiming90pc, fig.cap= "Mean state of charge at beginning of charge sequence by time of day (chargeType corrected, values > 90% removed)"}

dataDT <- firstCleanDT[SoC_percent < 90] # need this for caption counts
plotDT <- dataDT[,
                 .(meanChargePC = mean(SoC_percent),
                         nEVs = uniqueN(dvID)), 
                 keyby = .(qHour, weekdays, chargeTypeCorrected)]
...
#addPeaks(p)
ggsave(paste0(here::here(), "/docs/plots/SOC_Time_when_charging_begins_90pcObs.png"))
```

```{r SoCplotTimingAll, fig.cap="Mean state of charge at beginning of charge sequence by time of day (chargeType corrected, all observations)"}
# rebuild for all data
...
myCap <- paste0(cleanCap,
                 "\n First charging observation in a sequence, corrected charge type, all observations",
                "\n n observations: ", tidyNum(nrow(firstCleanDT)),
                 "\n n EVs: ", uniqueN(firstCleanDT$dvID)
                 )
...
yMax <- max(plotDT$meanChargePC)
addPeaks(p)
ggsave(paste0(here::here(), "/docs/plots/SOC_Time_when_charging_begins_allObs.png"))

```

Finally, Figure \@ref(fig:SoCplotTimingAll) shows the mean % charge at the time of first charging observation in a sequence using the cleaned and corrected inferred start/end of charging sequence data. This includes _all_ events, even the brief 'top-up' early morning events to indicate when the stored energy is available at the start of a charge sequence.
...
Making the assumption that each EV has a `r batteryCapacity` kWh battery we can calculate the _available_ kWh at the start of each charge for V2G purposes. This is shown in Table \@ref(tab:v2gEstimates) and Figure \@ref(fig:v2gEstimates).

```{r v2gEstimates, fig.cap= "Estimates of available kWh by time period"}

dt <- dt[, kWh := SoC_percent/100 * batteryCapacity]

t <- dt[, .(meankWh = mean(kWh),
       sdkWh = sd(kWh),
       nObs = .N,
       nEVS = uniqueN(dvID)), keyby = .(chargeType,peakPeriod)]

kableExtra::kable(t, caption = "Estimated kWh available by time period and charge type", digits = 2) %>%
  kable_styling()

p <- ggplot2::ggplot(dt, aes(x = peakPeriod, y = kWh, fill = chargeType)) +
  geom_boxplot(position = "dodge") +
  scale_fill_discrete(name = "Charge type") +
  labs(x = "Time period",
       y = "kWh available",
       caption = paste0("Data source: Flip The Fleet (cleaned, corrected data)",
                        min(dt$date) , " - ", max(dt$date),
                        "\nFirst charging observation in a sequence, corrected charge type, state of charge < 90%",
                        "\n n observations: ", nrow(dt),
                        "\n n EVs: ", uniqueN(dt$dvID)
                        )
       )
p
...
These show, for example, that the mean kWh available during the evening peak period is `r dt[chargeType %like% "Standard" & peakPeriod %like% "Evening", round(mean(kWh),2)]` with a range of `r dt[chargeType %like% "Standard" & peakPeriod %like% "Evening", round(min(kWh),2)]` to `r dt[chargeType %like% "Standard" & peakPeriod %like% "Evening", round(max(kWh),2)]` kWh. We can also see that similar values are available in the morning peak period but further analysis would be required to estimate how much of this could be used during the period without impacting subsequent use. Other analyses could include:
...
p <- p + labs(x = "State of charge when charging ends (%)")

ggplot2::ggsave(paste0(pPath,"/SoCplotEnd.png"), p ) # for PCE report
...
> The remaining results in this section are experimental and preliminary. Use with care (if at all) or just skip to the next section (\@ref(imputePower)).
...
Figure \@ref(fig:rapidPowerPlotDT) repeats Figure \@ref(fig:nonRapidPowerPlotDT) but shows the mean charging demand in kW calculated across all observations after setting standard charge observations to 0 kW. Again, the kW load due to the EVs follows essentially the same shape as the charging event counts shown above and the low mean value should remind us that rapid charging was relatively rare in the data.
...
## Imputing power demand {#imputePower}
...
### Method 1: Imptation based on sample charging proportions

> Warning: we think this method is incorrect. Do not use results in this section. Jump to Section \@ref(kw2)
...
Based on Table \@ref(tab:cleanObsChargingPower), let us assume:

 * standard charging = `r round(medianStd, 2)` kW (median)
 * rapid charging = `r round(medianRap, 2)` kW (median)

We can now calculate the mean kW demand per EV per time period by multiplying these values by the appropriate % of EVs who were charging by charge type (the data used to construct \@ref(fig:pcCharging)). The results of doing so are shown in \@ref(fig:setPower). 

```{r setPower, fig.caption = "Estimated mean kW"}
evPcChargingPlotDT[, sampleCarkW := ifelse(chargeType == "Standard charging",
                           meanPC * medianStd,
                           NA)
         ]
evPcChargingPlotDT[, sampleCarkW := ifelse(chargeType == "Rapid charging",
                           meanPC * medianRap,
                           sampleCarkW)
         ]

p <- ggplot2::ggplot(evPcChargingPlotDT[!(chargeType %like% "Not")], aes(x = qHour, y = sampleCarkW, colour = chargeType)) +
  geom_point() +
  scale_color_discrete(name = "Charge Type") +
  labs(y = "Estimated mean kW per EV per quarter hour", x = "Time of day") +
  facet_grid(weekdays ~ .)

yMin <- min(evPcChargingPlotDT$sampleCarkW, na.rm = TRUE)
yMax <- max(evPcChargingPlotDT$sampleCarkW, na.rm = TRUE)
p <- addPeaks(p)

ggplot2::ggsave(paste0(pPath,"/imputedkWperEV.png"), p ) # for PCE report
 ...
 * despite the relative rarity of rapid charging, their much higher power demand produces a much higher mean kW load on the network, especially during the day. Note that the level of rapid charging between 00:00 and 08:00 on weekdays seems unexpected although it is reflected in Figure \@ref(fig:pcCharging) which shows that it is based on `r uniqueN(cleanDT[chargeType %like% "Rapid" & peakPeriod %like% "Early", dvID])` different EVs. Given the relatively low number of EVs reporting rapooid charging in this time period, this result should be treated with caution. 

### Method 2: Half-hourly aggregation {#kw2}

In this method we:

 * set the dateTime of each observation to _exactly_ 1 minute by setting dateTime_1min =  floor(dateTime) - so e.g. 2018-10-01 20:00:59 -> 2018-10-01 20:00. This _may_ create multiple observations per EV where more than 1 observation was taken within a given minute;
...
```{r make1minkW, fig.cap="Histogram of number of observations per aggregated floor(1 minute) date time per EV"}

cleanDT[, r_dateTime1MinFl := lubridate::floor_date(r_dateTime, "1 min")]
cleanDT[, r_dateTime1MinR := lubridate::round_date(r_dateTime, "1 min")]

minDT <- cleanDT[, .(meankW = mean(charge_power_kw),
                     nObs = .N), # check
                 keyby = .(dvID, r_dateTime1MinFl, chargeType, peakPeriod, weekdays)]

t <- summary(minDT)

kableExtra::kable(t, caption = "Summary of floor(1 minute) aggregate data") %>%
  kable_styling()

ggplot2::ggplot(minDT, aes(x = nObs)) +
  geom_histogram()
```
...
```{r chargingKwProfile, fig.cap = "Half-hourly mean kW for all EVs - testing for missing data"}

minDT[, r_dateTime30MinFl := lubridate::floor_date(r_dateTime1MinFl, "30 mins")]

halfHourDT <- minDT[, .(sumkWObs = sum(meankW), 
                  nObs = .N), 
              keyby = .(dvID, r_dateTime30MinFl, chargeType, peakPeriod, weekdays)]

halfHourDT[, meankW := sumkWObs/30] # as we would expect 30 obs in a half hour - we might have more or less depending
halfHourDT[, hms := hms::as.hms(r_dateTime30MinFl)]
plotDT <- halfHourDT[, .(meankW = mean(meankW),
                         sumkW = sum(meankW)), 
                     keyby = .(chargeType, r_dateTime30MinFl, dvID)]

ggplot2::ggplot(plotDT, aes(x = r_dateTime30MinFl, y = dvID, fill = meankW)) +
  geom_tile() +
  facet_grid(. ~ chargeType) +
  scale_fill_continuous(low = "green", high = "red") +
  labs(x = "Date", y = "Mean kw in the half hour")
```
...
## Estimating energy consumption {#estimatekWh}
...
### Method 1: proportions charging

> Warning: we think this method is incorrect. Do not use results in this section. Jump to Section \@ref(kwh2)

In the first method, we take the mean kW per quarter hour per EV values reported in \@ref(fig:setPower) and divide them by 4 to give mean kWh per quarter hour per EV.

```{r estimatekWh}

evPcChargingPlotDT[, sampleCarkWh := sampleCarkW/4] # q hours
# 
# # 1: all obs at min level
# sum1kWDT <- cleanDT[, .(sumKw = sum(charge_power_kw), nObs = .N), keyby = .(chargeType)]
# sum1kWDT[, meankW := sumKw/nObs] # 1 minute obs...
# sum1kWDT[, sumkWh := 60*meankW] # yes? 1 minute obs...
# totkWh <- sum(sum1kWDT$sumkWh)
# sum1kWDT[, pckWh := 100*(sumkWh/totkWh)]
# 
# # 2: q hour level
# meanQHourkWDT <- cleanDT[, .(meanKw = mean(charge_power_kw), nObs = .N), keyby = .(chargeType, r_dateTimeQhour)]
# 
# sum2kWDT <- meanQHourkWDT[, .(sumKw = sum(meanKw), nObs = .N), keyby = .(chargeType)]
# sum2kWDT[, meankW := sumKw/nObs] # 1 minute obs...
# sum2kWDT[, sumkWh := 60*meankW] # yes? 1 minute obs...
# totkWh <- sum(sum2kWDT$sumkWh)
# sum2kWDT[, pckWh := 100*(sumkWh/totkWh)]
# 
# # these produce roughly the same result as you'd expect.

# t <- sum1kWDT[!(chargeType %like% "Not"), .("Charge type"= chargeType, 
#                   "Mean kW" = meankW, "Sum kWh" = sumkWh,
#                   "% consumption" = pckWh)]

totalWdaykWh <- sum(evPcChargingPlotDT[weekdays == "Weekdays"]$sampleCarkWh, na.rm = TRUE)
totalWekWh <- sum(evPcChargingPlotDT[weekdays == "Weekends"]$sampleCarkWh, na.rm = TRUE)


t <- evPcChargingPlotDT[!(chargeType %like% "Not"), 
                        .(totalMeanKWh = sum(sampleCarkWh)), 
                        keyby = .(weekdays, chargeType)]

t[weekdays == "Weekdays", totalMeanKWh_pc := 100*(totalMeanKWh/totalWdaykWh)]
t[weekdays == "Weekends", totalMeanKWh_pc := 100*(totalMeanKWh/totalWekWh)]

kableExtra::kable(t[, .("Day" = weekdays, "Charge type" = chargeType,
                        "Total mean kWh" = totalMeanKWh,
                        "% total mean kWh (within day)" = totalMeanKWh_pc)], digits = 2,
                  caption = "Estimated mean daily kWh by charge type (all observations)") %>%
  kable_styling()
```

Overall we estimate that `r round(100*t[chargeType %like% "Standard", sum(totalMeanKWh)]/sum(t$totalMeanKWh),2)`% of electricty consumption for EVs is via standard charging. Table \@ref(tab:estimatekWh) reports the estimated mean daily kWh consumed during charging on weekdays and weekends and the % within weekdays and weekends. The results indicate little difference between weekdays and weekends in terms of the % of kWh consumption which is standard vs rapid charging. 

```{r estimatekWhByPeak, fig.cap = "% total mean kWh consumption per EV"}
# repeat for peak
t <- evPcChargingPlotDT[!(chargeType %like% "Not"), 
                        .(totalMeanKWh = sum(sampleCarkWh)), 
                        keyby = .(weekdays, peakPeriod, chargeType)]

st <- evPcChargingPlotDT[!(chargeType %like% "Not"), 
                        .(dailyMeanKWh = sum(sampleCarkWh)), 
                        keyby = .(weekdays)]

setkey(t, weekdays, peakPeriod)
setkey(st, weekdays)

tt <- t[st]
tt[, totalMeanKWh_pc := 100*(totalMeanKWh/dailyMeanKWh)]

kableExtra::kable(tt[, .("Day" = weekdays, 
                         "Peak period" = peakPeriod,
                         "Charge type" = chargeType,
                        "Total mean kWh" = totalMeanKWh,
                        "% total mean kWh (within day)" = totalMeanKWh_pc)], digits = 2,
                  caption = "Estimate mean kW and kWh by charge type and peak period (all observations)") %>%
  kable_styling()

p <- ggplot2::ggplot(tt, aes(x = peakPeriod, y = totalMeanKWh_pc, fill = chargeType)) +
  geom_col() +
  scale_fill_discrete(name = "Charge type") +
  facet_grid(. ~ weekdays) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  labs(x = "Period", y = "% of total mean kWh")

ggplot2::ggsave(paste0(pPath,"/estimatekWhByPeak.png"), p ) # for PCE report
p
```

Table \@ref(tab:estimatekWhByPeak) and Figure \@ref(tab:estimatekWhByPeak) repeats this analysis but shows the results by peak period. Clearly rapid charging during the day dominates energy (kWh) consumption with standard charging during the evening peak being responsible for a mere 3-4% of the energy consumed by EV charging.

### Method 2: Aggregation {#kwh2}

Here we take the floor(1 min) data described in Section \@ref(kw2) and:

 * divide this by 2 to give the mean kWh per date-half-hour
 
```{r makeHalfHourlyKWh}

halfHourDT[, kWh := meankW/2] # as they are half-hours

# plot for sense
p <- ggplot(halfHourDT, aes(x = r_dateTime30MinFl, fill = kWh, y = dvID)) +
  geom_tile() +
  facet_grid(. ~ chargeType) +
  scale_fill_continuous(low = "green", high = "red") +
  labs(y = "kWh per half hour", x = "Time of day",
       caption = "Sense plot: all kWh per half hour observations for all EVs") 
p
```
...
Figure \@ref(fig:plotKWhWeekdays) shows the results of doing this for weekends and weekdays. Note that the totals have been 'normalised' to allow for the different numbers of weekdays vs weekend days but we make no checks as to whether the 'right' number of weekdays and weekend days are represented in the data (e.g. where we received no data due to inactivity - see above). This shows that the patterns of weekday vs weekend energy consumption are  similar with overnight standard charging dominating. Consumption during morning peak periods is low but slightly higher in the evening peaks as the charging timing results above would imply. Rapid charging plays a larger role in day-time energy consumption but it's rarity means that despite higher power demand (kW) it's energy impact (kWh) is relatively low in this sample.

```{r plotKWhWeekdays, fig.cap = "Total normalised kWh by weekday and charge type"}
# sum kwH within categories we care about
tWD <- halfHourDT[!(chargeType %like% "Not"), .(sumkWh = sum(kWh)), keyby = .(chargeType, peakPeriod, weekdays)]

# normalise to ndays
tWD[, nDays := ifelse(weekdays == "Weekdays", 5, 2)]

tWD[, sumkWhNdays := sumkWh/nDays]

ggplot(tWD, aes( x = peakPeriod, y = sumkWhNdays, fill = chargeType)) + 
  geom_col() +
  facet_grid(weekdays ~ .) +
  scale_fill_discrete(name = "Charge Type")+
  labs(y = "Total kWh (normalised)", x = "Time period")
```



```{r plotKWh, fig.cap="% total kWh by charge type and peak period"}
t <- halfHourDT[!(chargeType %like% "Not"), .(sumkWh = sum(kWh)), keyby = .(chargeType, peakPeriod)]

totkWh <- sum(t$sumkWh)

t[, pckWh := 100 * (sumkWh/totkWh)]

ggplot(t, aes( x = peakPeriod, y = pckWh, fill = chargeType)) + 
  geom_col() +
  scale_fill_discrete(name = "Charge Type")+
  labs(y = "% total kWh drawn", x = "Time period")

kableExtra::kable(t[, .("Time period" = peakPeriod,
                        "Charge Type" = chargeType, 
                        "Total kWh" = sumkWh,
                        "% of total kWh" = pckWh)][order(`Charge Type`)], digits = 2, caption = "Total kWh per period by charge type") %>%
  kable_styling()

message("Summary % total kWh for PCE paper")
t[, .("% of total kWh" = sum(pckWh)), keyby = .(chargeType)]

dailykWhDT <- halfHourDT[, .(sumkWh = sum(kWh)), keyby = .(date = lubridate::date(r_dateTime30MinFl), dvID, chargeType)]

dailykWhDT[sumkWh > 0, .("Mean daily kWh where any charging" = mean(sumkWh),
                         median = median(sumkWh),
                         min = min(sumkWh),
                         max = max(sumkWh)), keyby = .(chargeType)]

pcStdOffPeak <- t[chargeType %like% "Standard" & 
                    (peakPeriod == "Late evening" | peakPeriod == "Early morning"),
                    sum(pckWh)]

pcStdDay <- t[chargeType %like% "Standard" & 
                    (peakPeriod == "Day time"),
                    sum(pckWh)]

pcRapidDay <- t[chargeType %like% "Rapid" & 
                    (peakPeriod == "Day time"),
                    sum(pckWh)]

pcStdEveningPeak <- t[chargeType %like% "Standard" & 
                    (peakPeriod == "Evening peak"),
                    sum(pckWh)]
```

Table \@ref(tab:plotKWh) and Figure \@ref(tab:plotKWh) summarise the energy consumption across peak periods without weekday disaggregation. This shows that overall `r round(pcStdOffPeak)`% of the energy drawn was overnight off-peak standard charging. Day time standard charging was `r round(pcStdDay)`% of the total and day-time rapid only `r round(pcRapidDay)`%. Evening peak standard charging was `r round(pcStdEveningPeak)`% of total energy.
...
Based on Table \@ref(tab:cleanObsChargingPower), we can repeat the process used in Section \@ref(estimatekWh) by estimating:
 ...
 * Car ownership 2013 (http://archive.stats.govt.nz/Census/2013-census/profile-and-summary-reports/quickstats-transport-comms/number-motor-vehicles.aspx):
    ...
    * `r 100*oneCar`% have 1
    * `r 100*twoCar`% have 2
    * `r 100*threeCar`% have 3+ (we can treat this as 3)

We can now use the results shown in Figure \@ref(fig:pcCharging) to construct a number of EV uptake scenarios such as:

 * All households with >= 1 car switch one of them (or the one they have) to an EV (scenario 1) and their charging behaviour is exactly as reported in Figure \@ref(fig:pcCharging)
...
## Scenario 1: All households with >= 1 car switch one of them (or the one they have) to an EV

Under this scenario (Figure \@ref(fig:scaledCharging1)) we can see demand increases by ~ 500 MW during the late evening and overnight due to standard charging which tends to avoid peak periods. On the other hand rapid charging, although less frequent (see Figure \@ref(fig:pcCharging)) has a bigger effect due to it's larger power draw.

```{r scaledCharging1, fig.cap = "Estimated MW demand under scenario 1 (1 ICE <-> EV substitute for all households with cars)"}
evPcChargingPlotDT[, carkW := ifelse(chargeType %like% "Standard", 
                          carHouseholds * meanPC * medianStd,
                          NA)]
evPcChargingPlotDT[, carkW := ifelse(chargeType %like% "Rapid", 
...
p <- ggplot2::ggplot(evPcChargingPlotDT[!(chargeType %like% "Not")], aes(x = qHour, y = carkW/1000, colour = chargeType)) +
  geom_point() +
  scale_color_discrete(name = "Charge Type") +
  labs(y = "Estimated mean MW demand", x = "Time of day") +
  facet_grid(weekdays ~ .)

yMin <- min(evPcChargingPlotDT$carkW/1000, na.rm = TRUE)
yMax <- max(evPcChargingPlotDT$carkW/1000, na.rm = TRUE)
addPeaks(p)

t <- evPcChargingPlotDT[!(chargeType %like% "Not"), .("Mean estimated MW" = mean(carkW/1000, na.rm = TRUE)), keyby = .(peakPeriod, chargeType)]
...
As we would expect this plot shows exactly the same distribution as Figure \@ref(fig:setPower).
...
```{r genDataPrep}
genDataDT <- genDataDT[, season := ifelse(month > 2 & month < 6, "Autumn", "Summer")]
genDataDT <- genDataDT[, season := ifelse(month > 5 & month < 9, "Winter", season)]
genDataDT <- genDataDT[, season := ifelse(month > 8 & month < 12, "Spring", season)]

#table(genDataDT$month, genDataDT$season)

genPlotDT <- genDataDT[, .(nHalfHours = uniqueN(rDateTime),
                 sumkWh = sum(kWh)), # sum over all gen types
                 keyby = .(season, weekdays, halfHour, peakPeriod)]
...
Figure \@ref(fig:convertEVtoMWhSum) shows the result of this calculation for the summer period. This uses wholesale generation data from January, February and December 2018 and the EV charging data for the study period as reported above. We assume that the EV charging behaviour is not seasonally affected although this may not necessarily be true in practice. Note that the rarity of rapid charging causes some zero values for this charge type.

```{r convertEVtoMWhSum, fig.cap = "Actual wholesale generation and estimated energy consumption under Scenario 1 for the study period during the summer."}

evPcChargingPlotDT <- evPcChargingPlotDT[, halfHour := hms::trunc_hms(qHour, 30*60)]

evHalfHourDT <- evPcChargingPlotDT[, .(meankW = mean(carkW)), keyby = .(chargeType, halfHour, peakPeriod, weekdays)]
...
dt <- rbind(genPlotDT[season == "Summer",.(weekdays, halfHour, kWh, chargeType, peakPeriod)], evHalfHourDT[,.(weekdays, halfHour, kWh, chargeType, peakPeriod)])

p <- ggplot2::ggplot(dt[!(chargeType %like% "Not")], aes(x = halfHour, y = kWh/1000, colour = chargeType)) +
  geom_point() +
  scale_color_discrete(name = "Key") +
  labs(x = "Time of Day", y = "Mean MWh per half hour") +
  facet_grid(weekdays ~ .)

yMin <- min(dt$kWh/1000, na.rm = TRUE)
yMax <- max(dt$kWh/1000, na.rm = TRUE)
addPeaks(p)

t <- dcast(dt[!(chargeType %like% "Not")], weekdays + halfHour + peakPeriod ~ chargeType, value.var = "kWh", fun=sum)
#head(t)

t[, rapid_pc := 100*(`Rapid charging`/`Wholesale generation`)]
t[, std_pc := 100*(`Standard charging`/`Wholesale generation`)]

pct <- t[, .(meanRapidPC = mean(rapid_pc, na.rm = TRUE),
      meanStdPC = mean(std_pc, na.rm = TRUE)), keyby = .(weekdays, peakPeriod)]
kableExtra::kable(pct, digits = 2, caption = "Mean % of observed wholesale generation by peak period (summer)") %>% 
  kable_styling()

dt[, chargeType := forcats::lvls_reorder(chargeType, 4:1)] # 4 levels (includes Not)
ggplot2::ggplot(dt[!(chargeType %like% "Not")], aes(x = halfHour, y = kWh/1000, fill = chargeType)) +
  geom_col() +
...
Figure \@ref(fig:convertEVtoMWhWin) shows the result of this calculation for the summer period. This uses wholesale generation data from June, July and August 2018 and the EV charging data for the study period as reported above. as we do not have EV charging data for a winter period, we assume that the EV charging behaviour is not seasonally affected although this may not necessarily be true in practice. Note that the rarity of rapid charging causes some zero values for this charge type.

```{r convertEVtoMWhWin, fig.cap = "Actual wholesale generation and estimated energy consumption under Scenario 1 during the winter."}

dt <- rbind(genPlotDT[season == "Winter",.(weekdays, halfHour, kWh, chargeType, peakPeriod)], evHalfHourDT[,.(weekdays, halfHour, kWh, chargeType, peakPeriod)])
...
pct <- t[, .(meanRapidPC = mean(rapid_pc, na.rm = TRUE),
      meanStdPC = mean(std_pc, na.rm = TRUE)), keyby = .(weekdays, peakPeriod)]
kableExtra::kable(pct, digits = 2, caption = "Mean % of observed wholesale generation by peak period (winter)") %>% 
  kable_styling()
...
```{r createAllHalfHoursDT}
# we know there are missing half-hours in the EV data where we didn't receive charging or driving observations
# these cannot be missing from the wholesale gen data (except for DST breaks)
# so we cna use the wholesale gen data as a spine to fill in the missing half-hours
# better to do that in half-hours than minutes (way quicker)

halfHourSpineDT <- genDataDT[, .(wholesaleGenkWh = sum(kWh),
                                 nGenerators = .N),
                             keyby = .(rDateTime = lubridate::floor_date(rDateTime, unit = "30 mins"), weekdays, peakPeriod)]

setkey(halfHourSpineDT, rDateTime)
setkey(halfHourDT, r_dateTime30MinFl)

halfHourAllDT <- halfHourDT[halfHourSpineDT]

plotDT <- halfHourAllDT[, .(wholesaleGenkWh = mean(wholesaleGenkWh), # as there will be duplicates where the EVs matched
                            evkWh = mean(kWh, na.rm = TRUE)
                            ), keyby = .(r_dateTime30MinFl)]

ggplot2::ggplot(plotDT, aes(x = r_dateTime30MinFl, y = wholesaleGenkWh)) +
  geom_line()

ggplot2::ggplot(plotDT, aes(x = r_dateTime30MinFl, y = evkWh)) +
  geom_line()

plotDT <- halfHourDT[, .(evkWh = mean(kWh, na.rm = TRUE)
                            ), keyby = .(r_dateTime30MinFl, dvID, chargeType)]

ggplot2::ggplot(plotDT, aes(x = r_dateTime30MinFl, y = dvID, fill = evkWh)) +
  geom_tile() +
  facet_grid(. ~ chargeType) +
